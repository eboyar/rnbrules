...
public class CustomFlyingGoal extends Goal {
	...
	public void tick() {
		...
		if (this.takingOff) {
			...
			RayTraceResult mop =
				this.pixelmon.level.clip(
						new RayTraceContext(
							new Vector3d(this.pixelmon.getX(), box.minY, this.pixelmon.getZ()),
							new Vector3d(this.pixelmon.getX() + this.pixelmon.getDeltaMovement().x() * (double) 100.0F, box.minY, this.pixelmon.getZ() + this.pixelmon.getDeltaMovement().z() * (double) 100.0F),
							BlockMode.COLLIDER,
			...
			if (mop.getType() == Type.MISS) {
				mop = this.pixelmon.level.clip(
						new RayTraceContext(
							new Vector3d(this.pixelmon.getX(), box.maxY, this.pixelmon.getZ()),
							new Vector3d(this.pixelmon.getX() + this.pixelmon.getDeltaMovement().x() * (double) 100.0F, box.maxY, this.pixelmon.getZ() + this.pixelmon.getDeltaMovement().z() * (double) 100.0F),
							BlockMode.COLLIDER,
							FluidMode.SOURCE_ONLY,
							this.pixelmon
						)
			...
			if (this.hasLandingSpot()) {
				if (mop.getType() == Type.MISS) {
					...
					this.pixelmon.setDeltaMovement(
							this.pixelmon.getDeltaMovement().x() + (Math.signum(d0) - this.pixelmon.getDeltaMovement().x()) * (double) 0.1F,
							this.pixelmon.getDeltaMovement().y() + (Math.signum(d1) * (double) 0.7F - this.pixelmon.getDeltaMovement().y()) * (double) 0.1F,
							this.pixelmon.getDeltaMovement().z() + (Math.signum(d2) - this.pixelmon.getDeltaMovement().z()) * (double) 0.1F
						);
	...
	private void checkForLandingSpot() {
		...
		if (this.currentFlightTarget == null || this.pixelmon.getRandom().nextInt(30) == 0) {
			this.currentFlightTarget = new BlockPos(
				(int) (this.pixelmon.getX() + this.pixelmon.getDeltaMovement().x() * (double) 200.0F + (double) this.pixelmon.getRandom().nextInt(10) - (double) 5.0F),
				0,
				(int) (this.pixelmon.getZ() + this.pixelmon.getDeltaMovement().z() * (double) 200.0F + (double) this.pixelmon.getRandom().nextInt(10) - (double) 5.0F)
			);
	...
	private void maintainFlight(boolean hasObstacle) {
		...
		if (hasObstacle || this.wingBeatTick >= this.nextWingBeat) {
			...
			this.nextWingBeat =
				this.pixelmon.getFlyingParameters().getFlapRate() +
				(int) (Math.random() * 0.4 * (double) this.pixelmon.getFlyingParameters().getFlapRate() - 0.2 * (double) this.pixelmon.getFlyingParameters().getFlapRate());
			this.pixelmon.travel(
					new Vector3d(
						(double) 0.0F,
						(double) 0.0F,
						(double) (4.0F + ((float) this.pixelmon.getPokemon().getStat(BattleStatsType.SPEED) / 100.0F) * this.pixelmon.getFlyingParameters().getFlySpeedModifier())
					)
				);
	...
	private void takeOff() {
		...
		this.pixelmon.level.playSound(
				(PlayerEntity) null,
				this.pixelmon.getX(),
				this.pixelmon.getY(),
				this.pixelmon.getZ(),
				(SoundEvent) SoundRegistration.TAKE_OFF.get(),
