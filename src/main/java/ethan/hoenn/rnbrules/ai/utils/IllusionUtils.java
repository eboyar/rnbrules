...
public class IllusionUtils {
	...
	public static boolean shouldAIKnowAboutIllusion(PixelmonWrapper pw, PixelmonWrapper target, boolean debugMode) {
		...
		if (pw.lastAttack != null && !pw.isFirstTurn()) {
			...
			if (basePower > 0) {
				...
				debugLog("AI used damaging move " + pw.lastAttack.getActualMove().getAttackName() + 
						 " (base power: " + basePower + ") - should know about illusion on " + 
						 target.getSpecies().getLocalizedName(), debugMode);
				return true;
			} else {
				
				debugLog("AI used non-damaging move " + pw.lastAttack.getActualMove().getAttackName() + 
						 " (base power: " + basePower + ") - remaining oblivious to illusion on " + 
						 target.getSpecies().getLocalizedName(), debugMode);
				return false;
			}
		}
		...
		debugLog("No previous attack recorded - AI doesn't know about illusion on " + 
				 target.getSpecies().getLocalizedName(), debugMode);
		return false;
	}
	...
	public static void evaluateChoiceWithIllusionSwitch(PixelmonWrapper pw, MoveChoice choice, 
			PixelmonWrapper targetWithIllusion, PixelmonWrapper actualDisguisedPokemon, 
			BattleController bc, Runnable normalEvaluation, boolean debugMode) {
		...
		try {
			...
			SwitchUtils.executeWithTemporarySwitch(targetWithIllusion, actualDisguisedPokemon, bc, 
					targetWithIllusion.getStatuses(), () -> {
						
				
				normalEvaluation.run();
				
				debugLog("Evaluated against disguised Pokémon " + actualDisguisedPokemon.getSpecies().getLocalizedName() + 
						 " with score: " + choice.weight, debugMode);
			}, debugMode);
