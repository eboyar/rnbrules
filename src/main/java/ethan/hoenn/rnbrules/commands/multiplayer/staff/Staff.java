...
public class Staff {
    ...
    public static void register(CommandDispatcher<CommandSource> dispatcher) {
                ...
                .then(Commands.literal("ban")
                    .then(Commands.argument("player", EntityArgument.player())
                        .then(Commands.argument("reason", MessageArgument.message())
                            .executes(context -> banPlayer(context, false))
                            .then(Commands.literal("ip")
                                .executes(context -> banPlayer(context, true))
                            )
                        )
                    )
                )
        ...
        dispatcher.register(
            Commands.literal("staff")
                .requires(Staff::hasStaffPermission)
                .redirect(dispatcher.getRoot().getChild("st"))
        );
    }
    ...
    private static int banPlayer(CommandContext<CommandSource> context, boolean ipBan) {
        try {
            ...
            ProfileBanEntry banEntry = new ProfileBanEntry(
                ...
                null, 
                staff.getDisplayName().getString(), 
                null, 
                reason.getString()
            );
            ...
            if (ipBan) {
                ...
                IPBanEntry ipBanEntry = new IPBanEntry(
                    target.getIpAddress(), 
                    null, 
                    staff.getDisplayName().getString(), 
                    null, 
            ...
            broadcastToStaff(context.getSource().getServer(), 
                TextFormatting.RED + staff.getDisplayName().getString() + " " + banType + " " + 
                target.getDisplayName().getString() + " for: " + reason.getString());
                
            context.getSource().sendSuccess(
                new StringTextComponent(TextFormatting.GREEN + "Successfully " + banType + " " + target.getDisplayName().getString()), 
                true
            );
    ...
    private static int kickPlayer(CommandContext<CommandSource> context) {
        try {
            ...
            broadcastToStaff(context.getSource().getServer(), 
                TextFormatting.YELLOW + staff.getDisplayName().getString() + " kicked " + 
                target.getDisplayName().getString() + " for: " + reason.getString());
                
            context.getSource().sendSuccess(
                new StringTextComponent(TextFormatting.GREEN + "Successfully kicked " + target.getDisplayName().getString()), 
                true
            );
    ...
    private static int mutePlayer(CommandContext<CommandSource> context) {
        try {
            ...
            target.sendMessage(new StringTextComponent(TextFormatting.RED + "You have been permanently muted by " + 
                staff.getDisplayName().getString() + ". Reason: " + reason.getString()), target.getUUID());
            
            
            broadcastToStaff(context.getSource().getServer(), 
                TextFormatting.RED + staff.getDisplayName().getString() + " permanently muted " + 
                target.getDisplayName().getString() + " for: " + reason.getString());
                
            context.getSource().sendSuccess(
                new StringTextComponent(TextFormatting.GREEN + "Successfully muted " + target.getDisplayName().getString()), 
                true
            );
    ...
    private static int tempMutePlayer(CommandContext<CommandSource> context) {
        try {
            ...
            target.sendMessage(new StringTextComponent(TextFormatting.RED + "You have been muted for " + durationStr + 
                " by " + staff.getDisplayName().getString() + ". Reason: " + reason.getString()), target.getUUID());
            
            broadcastToStaff(context.getSource().getServer(),
                TextFormatting.RED + staff.getDisplayName().getString() + " muted " + 
                target.getDisplayName().getString() + " for " + durationStr + ". Reason: " + reason.getString());
                
            context.getSource().sendSuccess(
                new StringTextComponent(TextFormatting.GREEN + "Successfully muted " + target.getDisplayName().getString() + " for " + durationStr), 
                true
            );
    ...
    private static int tempBanPlayer(CommandContext<CommandSource> context) {
        try {
            ...
            target.connection.disconnect(new StringTextComponent("Temporarily banned for " + durationStr + 
                " by " + staff.getDisplayName().getString() + ". Reason: " + reason.getString()));
            
            
            broadcastToStaff(context.getSource().getServer(), 
                TextFormatting.RED + staff.getDisplayName().getString() + " temporarily banned " + 
                target.getDisplayName().getString() + " for " + durationStr + ". Reason: " + reason.getString());
                
            context.getSource().sendSuccess(
                new StringTextComponent(TextFormatting.GREEN + "Successfully banned " + target.getDisplayName().getString() + " for " + durationStr), 
                true
            );
    ...
    private static int unmutePlayer(CommandContext<CommandSource> context) {
        try {
            ...
            target.sendMessage(new StringTextComponent(TextFormatting.GREEN + "You have been unmuted by " + 
                staff.getDisplayName().getString()), target.getUUID());
            
            
            broadcastToStaff(context.getSource().getServer(), 
                TextFormatting.GREEN + staff.getDisplayName().getString() + " unmuted " + 
                target.getDisplayName().getString());
                
            context.getSource().sendSuccess(
                new StringTextComponent(TextFormatting.GREEN + "Successfully unmuted " + target.getDisplayName().getString()), 
                true
            );
