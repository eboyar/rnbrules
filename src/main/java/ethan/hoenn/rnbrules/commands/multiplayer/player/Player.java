...
public class Player {
    ...
    public static void register(CommandDispatcher<CommandSource> dispatcher) {        dispatcher.register(
            Commands.literal("pl")
                .then(Commands.literal("mute")
                    .then(Commands.argument("player", EntityArgument.player())
                        .executes(Player::togglePersonalMute)
                    )
                )
                .then(Commands.literal("unmute")
                    .then(Commands.argument("player", EntityArgument.player())
                        .executes(Player::unmutePlayer)
                    )
                )
                .then(Commands.literal("listmuted")
                    .executes(Player::listMutedPlayers)
                )
                .then(Commands.literal("muteall")
                    .executes(Player::toggleMuteAll)
                )
        );
        ...
        try {
            ...
            if (isMuted) {
                mutingPlayer.sendMessage(
                    new StringTextComponent(TextFormatting.GREEN + "You have muted " + 
                        TextFormatting.YELLOW + targetPlayer.getDisplayName().getString() + 
                        TextFormatting.GREEN + ". You will no longer see their chat messages."), 
                    mutingPlayer.getUUID()
                );
            } else {
                mutingPlayer.sendMessage(
                    new StringTextComponent(TextFormatting.GREEN + "You have unmuted " + 
                        TextFormatting.YELLOW + targetPlayer.getDisplayName().getString() + 
                        TextFormatting.GREEN + ". You will now see their chat messages again."), 
                    mutingPlayer.getUUID()
                );
            }
            ...
            source.sendFailure(new StringTextComponent(TextFormatting.RED + "Error: " + e.getMessage()));            return 0;
        }
    }
    ...
    private static int unmutePlayer(CommandContext<CommandSource> context) {
        ...
        try {
            ...
            if (!settingsManager.isPersonallyMuted(mutingPlayer.getUUID(), targetPlayer.getUUID())) {
                source.sendFailure(new StringTextComponent(TextFormatting.RED + "You have not muted " + 
    ...
    private static int listMutedPlayers(CommandContext<CommandSource> context) {
        ...
        if (mutedPlayers.isEmpty()) {
            player.sendMessage(
                new StringTextComponent(TextFormatting.YELLOW + "You have not muted any players."), 
                player.getUUID()
            );
            return 1;
        }
    ...
    private static int toggleMuteAll(CommandContext<CommandSource> context) {
        ...
        if (newSetting) {
            player.sendMessage(
                new StringTextComponent(TextFormatting.GREEN + "Mute All " + TextFormatting.YELLOW + "enabled" + 
                    TextFormatting.GREEN + ". You will only see critical messages."), 
                player.getUUID()
            );
        } else {
            player.sendMessage(
                new StringTextComponent(TextFormatting.GREEN + "Mute All " + TextFormatting.YELLOW + "disabled" + 
                    TextFormatting.GREEN + ". You will now see all chat messages."), 
                player.getUUID()
            );
        }
