												...
												.executes(context -> {
													...
													if (globalot.playerHasGlobalOT(player.getUUID(), encounterID)) {
														source.sendFailure(new StringTextComponent(
																TextFormatting.RED + "Failed to start encounter '" + encounterID + "'. The player has had this encounter before."));
														return 0;
													}
													...
													if (success) {
														...
														source.sendSuccess(new StringTextComponent(
																TextFormatting.GREEN + "Started encounter '" + encounterID + "' for " + player.getDisplayName().getString()), true);
													} else {
														source.sendFailure(new StringTextComponent(
																TextFormatting.RED + "Failed to start encounter '" + encounterID + "'. The player might not have any available Pokémon."));
													}
