							...
							.executes(context -> {
								try {
									...
									if (destinationName.equals("ALL")) {
										...
										context.getSource().sendSuccess(new StringTextComponent("Removed ALL ferry destinations from " + player.getDisplayName().getString()).withStyle(TextFormatting.GREEN), true);
										return 1;
									}
									...
									try {
										...
										context
											.getSource()
											.sendSuccess(
												new StringTextComponent("Removed ferry destination " + destination.getDisplayName() + " from " + player.getDisplayName().getString()).withStyle(TextFormatting.GREEN),
												true
											);
