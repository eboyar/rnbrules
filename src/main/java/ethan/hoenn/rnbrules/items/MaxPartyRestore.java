...
public class MaxPartyRestore extends Item {

	public MaxPartyRestore(Item.Properties properties) {
		super(properties);
	}
		...
		if (!world.isClientSide) {
			...
			if (stack.getDamageValue() != 10) {
				if (!gm.isPartOfAnyGauntlet(player.getUUID()) || (gm.isPartOfAnyGauntlet(player.getUUID()) && gm.isHealingAllowedForPlayer(player.getUUID()))) {
					...
					assert party != null;
					for (Pokemon pkm : party.getAll()) {
						if (pkm != null && (pkm.getHealth() < pkm.getMaxHealth() || !pkm.getMoveset().hasFullPP() || pkm.getStatus() != NoStatus.noStatus || pkm.isFainted())) {
							needsHealing = true;
							break;
						}
					}
					...
					if (!needsHealing) {
						player.sendMessage(new StringTextComponent("§7Your party is already fully healed!"), player.getUUID());
					} else {
						party.heal();
						player.sendMessage(new StringTextComponent("§aYour Pokémon have been fully healed!"), player.getUUID());
				...
				player.sendMessage(new StringTextComponent("§7You must refill your §5Max Party Restore§7 at a healer machine."), player.getUUID());
				//code for refills currently in gauntletlistener bc im lazy
			}
		}
