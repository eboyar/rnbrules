...
@OnlyIn(Dist.CLIENT)
public class AuroraCaveEnvironment extends BaseEnvironment {

	private static final Random RANDOM = new Random();
	private static final float MAX_PARTICLES_PER_TICK_BASE = 12.0F;
	...
	public AuroraCaveEnvironment() {
		super(Environment.AURORA_CAVE);
	}
	...
	public void update(float delta) {
		...
		if (Minecraft.getInstance().options.particles != ParticleStatus.MINIMAL) {
			spawnAuroraCaveParticles(player, delta);
		}

		if (soundTimer <= 0) {
			soundTimer = 400 + RANDOM.nextInt(200);
		} else {
			soundTimer--;
		}
	}
	...
	public boolean isInEnvironment(World world, BlockPos pos) {
		...
		PlayerEntity player = getClientPlayer();
		if (player != null) {
			if (ClientEnvironmentController.getCurrentClientEnvironment().equals(Environment.AURORA_CAVE)) {
				return true;
			}
		}
		return false;
	...
	private void spawnAuroraCaveParticles(PlayerEntity player, float delta) {
		...
		for (int i = 0; i < particlesToSpawn; i++) {
			...
			double oscillationFactor = 0.002 + (RANDOM.nextDouble() * 0.002);
			double riseSpeed = 0.035 + (RANDOM.nextDouble() * 0.02);
			double initialPhase = RANDOM.nextDouble() * Math.PI * 2.0;

			if (inWater) {
				riseSpeed *= 0.8D;
				oscillationFactor *= 0.9D;
			}
