...
public class EvolutionTask extends PokemonTask {
	...
	public void getConfig(ConfigGroup config) {
		...
		config.addEnum(
			...
			evoType,
			v -> evoType = v,
			NameMap.of(EvolutionType.ANY, EvolutionType.values())
	...
	private boolean matches(Evolution evolution) {
		...
		switch (evoType) {
			...
			case "interact":
				return this.evoType == EvolutionType.ITEM && evolution instanceof InteractEvolution && (item.isEmpty() || ((InteractEvolution) evolution).item.getItemStack().equals(item));
			case "ticking":
