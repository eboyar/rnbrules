...
public class BattleMoveTask extends PokemonTask {
	...
	public void getConfig(ConfigGroup config) {
		...
		config.addEnum("moveType", moveType, v -> moveType = v, NameMap.of(MoveType.ANY, MoveType.values()).nameKey(v -> "pixeltweaks.battle_move_type." + v.toString().toLowerCase()).create());
	}
	...
	public ITextComponent getAltTitle() {
		TranslationTextComponent title = new TranslationTextComponent(
			"ftbquests.task.pixelmon.battle_move.title",
		...
		if (this.cachedSpec != null) {
			title = new TranslationTextComponent(
				"ftbquests.task.pixelmon.battle_move.title_specific",
				new TranslationTextComponent("attack." + this.attack),
				new StringTextComponent(this.count > 1 ? " x" + this.count : ""),
				getPokemon()
			);
