...
public class CatchTask extends PokemonTask {
	...
	public void getConfig(ConfigGroup config) {
		...
		config.addEnum("type", getType, v -> getType = v, NameMap.of(GetType.ANY_EXCEPT_COMMANDS, GetType.values()).nameKey(v -> "pixeltweaks.catch_type." + v.name().toLowerCase()).create());
	}

	public void catchPokemon(TeamData team, Pokemon pokemon) {
		if (
			!team.isCompleted(this) &&
			(this.cachedSpec == null || this.cachedSpec.matches(pokemon)) &&
	...
	public void catchRaidPokemon(TeamData team, Pokemon pokemon) {
		if (
			...
			(getType == GetType.CATCH_OR_RAID || getType == GetType.RAID || getType == GetType.ANY || getType == GetType.ANY_EXCEPT_COMMANDS)
		) { //If it IS from raids
			team.addProgress(this, 1L);
		}
	}
	...
	public void onCommand(TeamData team, Pokemon pokemon) {
		if (!team.isCompleted(this) && (this.cachedSpec == null || this.cachedSpec.matches(pokemon) != this.invert) && (getType == GetType.COMMAND || getType == GetType.ANY)) { //If it is from commands
			team.addProgress(this, 1L);
		}
	}
	...
	public void onChristmas(TeamData team, Pokemon pokemon) {
		if (
			!team.isCompleted(this) &&
			(this.cachedSpec == null || this.cachedSpec.matches(pokemon) != this.invert) &&
			(getType == GetType.CHRISTMAS || getType == GetType.ANY || getType == GetType.ANY_EXCEPT_COMMANDS)
		) { //If it is from commands
			team.addProgress(this, 1L);
		}
	}
	...
	public void onFossil(TeamData team, Pokemon pokemon) {
		if (
			!team.isCompleted(this) &&
			(this.cachedSpec == null || this.cachedSpec.matches(pokemon) != this.invert) &&
			(getType == GetType.FOSSIL || getType == GetType.ANY || getType == GetType.ANY_EXCEPT_COMMANDS)
		) { //If it is from fossils
			team.addProgress(this, 1L);
		}
	}

	public void onEggHatch(TeamData team, Pokemon pokemon) {
		if (!team.isCompleted(this) && (this.cachedSpec == null || this.cachedSpec.matches(pokemon) != this.invert) && (getType == GetType.ANY || getType == GetType.ANY_EXCEPT_COMMANDS)) { //If it is from eggs
			team.addProgress(this, 1L);
		}
	}
}
