...
public class BringPokemonTask extends LocationTask {
	...
	public void getConfig(ConfigGroup config) {
		...
		config.addEnum("slot", this.slot, v -> this.slot = v, NameMap.of(0, new Integer[] { 0, 1, 2, 3, 4, 5, 6 }).nameKey(v -> "pixeltweaks.partyslot." + v).create(), 0);
		config.addInt(
			"x",
			this.x,
			v -> {
	...
	public ITextComponent getAltTitle() {
		TranslationTextComponent title = new TranslationTextComponent(
			"ftbquests.task." + this.getType().id.getNamespace() + '.' + this.getType().id.getPath() + ".title",
			new StringTextComponent(this.invert ? "anything but " : "a ").append(PokemonTask.getPokemon(this.cachedSpec)),
			getLocation()
		);
		return title;
	}
	...
	public boolean canSubmit(TeamData teamData, ServerPlayerEntity player) {
		if (this.ignoreDimension || this.dimension == player.getLevel().dimension()) {
