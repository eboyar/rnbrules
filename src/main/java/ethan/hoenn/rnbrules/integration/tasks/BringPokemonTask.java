...
public class BringPokemonTask extends LocationTask {
    ...
    public ITextComponent getAltTitle() {
        TranslationTextComponent title = new TranslationTextComponent("ftbquests.task." + this.getType().id.getNamespace() + '.' + this.getType().id.getPath() + ".title", new StringTextComponent(this.invert ? "anything but " : "a ").append(PokemonTask.getPokemon(this.cachedSpec)), getLocation());
        return title;
    }
