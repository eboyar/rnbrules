...
public class TaskListener {
		...
		if (event.getCause().equals(PokemonReceivedEvent.Constants.TRADE)) {
			...
			for (CatchTask task : catchTasks) {
				if (data.getProgress(task) < task.getMaxProgress() && data.canStartTasks(task.quest)) {
					...
					} else if (event.getCause().equals(PokemonReceivedEvent.Constants.COMMAND) || event.getCause().equals(PokemonReceivedEvent.Constants.GIFT_COMMAND)) {
	...
	private void moneyThing(EconomyEvent event) {
		...
		Scheduling.schedule(
			...
			() -> {
				...
				Scheduling.schedule(2, () -> antiOverflow.remove(event.getPlayer().getUUID()), false);
	...
	public void onLogin(PlayerEvent.PlayerLoggedInEvent event) {
		if (event.getPlayer() instanceof ServerPlayerEntity) updatePokedex((ServerPlayerEntity) event.getPlayer());
	}
	...
	public void onWipeout(BattleEndEvent event) {
		...
		if (event.getBattleController().isRaid() || event.getBattleController().isSimulation() || event.getBattleController().getPlayers().isEmpty()) return;
