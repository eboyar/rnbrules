...
public abstract class PokemonTask extends Task {
	...
	public Icon getAltIcon() {
		if (
			cachedSpec != null && cachedSpec.getValue(SpeciesRequirement.class).isPresent() && !this.cachedSpec.toString().isEmpty() && !this.cachedSpec.toString().split(" ")[0].equalsIgnoreCase("random")
		) {
	...
	public static ITextComponent getPokemon(PokemonSpecification spec) {
		...
		if (spec.getValue(PokeBallRequirement.class).isPresent()) {
			TranslationTextComponent ball = new TranslationTextComponent("item.pixelmon." + spec.getValue(PokeBallRequirement.class).get().getName().toLowerCase());
		...
		if (spec.getValue(SpeciesRequirement.class).isPresent()) {
			...
			if (spec.getValue(GenerationRequirement.class).isPresent()) {
				TranslationTextComponent newType = new TranslationTextComponent("pixeltweaks.lang.generation", spec.getValue(GenerationRequirement.class).get());
				componentList.add(newType);
			}
			if (spec.getValue(TypeRequirement.class).isPresent()) {
				TranslationTextComponent type = new TranslationTextComponent("type." + spec.getValue(TypeRequirement.class).get().getSecond().name().toLowerCase());
		...
		if (spec.getValue(PaletteRequirement.class).isPresent()) {
			TranslationTextComponent form = new TranslationTextComponent("pixelmon.palette." + spec.getValue(PaletteRequirement.class).get().toLowerCase());
