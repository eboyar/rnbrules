 ...
 */
package ethan.hoenn.rnbrules.integration.rewards;
...
public class PokelootReward extends Reward {
	...
	public void claim(ServerPlayerEntity player, boolean notify) {
		if (playSound) {
			player.playNotifySound(SoundRegistration.POKELOOT_OBTAINED.get(), SoundCategory.BLOCKS, 0.2F, 1.0F);
		}

		if (notify) {
			new DisplayRewardToastMessage(
				this.id,
				new TranslationTextComponent("ftbquests.reward.pixelmon.pokeloot.toast", this.count, new TranslationTextComponent("ftbquests.reward.pixelmon.pokeloot." + lootTier.name().toLowerCase())),
				Icon.getIcon("rnbrules:textures/gui/pokeloot/" + (lootTier.ordinal() + 1) + ".png")
			).sendTo(player);
		}
	...
	public Icon getAltIcon() {
		return Icon.getIcon("rnbrules:textures/gui/pokeloot/" + (lootTier.ordinal() + 1) + ".png");
	}

	@OnlyIn(Dist.CLIENT)
	@Override
	public IFormattableTextComponent getAltTitle() {
		return (new StringTextComponent(this.count + "x ")).append(
				new TranslationTextComponent("ftbquests.reward.pixelmon.pokeloot.title", new TranslationTextComponent("ftbquests.reward.pixelmon.pokeloot." + lootTier.name().toLowerCase()))
			);
	}
	...
	public void getConfig(ConfigGroup config) {
		...
		config.addEnum("loot_tier", lootTier, v -> lootTier = v, NameMap.of(LootTier.ONE, LootTier.values()).nameKey(v -> "ftbquests.reward.pixelmon.pokeloot." + v.name().toLowerCase()).create());
	}
